  ...
  return (
    <div className="bg-[#fff] w-[17vw] p-2 flex flex-col justify-between h-[100vh]">
      <div>
        ...
        <div className="flex flex-col gap-3 mt-4 font-[500] w-[95%]">
          {links.map(({ href, label, icon }) => {
            return (
              <Link
                href={href}
                className={`flex  text-[#575757] gap-3 p-2 transition duration-800 hover:bg-[#f96900] hover:text-white rounded-lg ${
                  pathname === href ? "bg-[#f96900] text-white" : "bg-[#fff]"
                }`}
              >
                <span>{icon}</span>
                <span>{label}</span>
